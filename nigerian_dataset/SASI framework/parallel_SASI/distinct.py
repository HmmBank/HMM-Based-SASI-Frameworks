...
def distinct_elements(csv_file):
    ...
    with open(csv_file, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # Number of transactions is before the last column
            nb_transactions = row[-2].strip()

            # Feature vector = columns between matricule and number of transactions
            vector = tuple(float(x) for x in row[1:-2])
