...
# ================================================
# 7. Saves dynamic RFM-LSTM features in ARFF files
# ================================================
def save_lstm_features(matricules, nb_transactions, R_time_series, F_time_series, M_time_series, dim_lstm, file_lstm, nb_epochs, batch_size, patience):
    ...
    # EarlyStopping
    early_stop = EarlyStopping(monitor='loss', patience=patience, min_delta=1e-4, restore_best_weights=True)

    # Training the model using the Adam optimizer
    autoencoder.compile(optimizer=Adam(), loss='mse')
    autoencoder.fit(X, X, epochs=nb_epochs, batch_size=batch_size, verbose=0, callbacks=[early_stop])

    # Generate the T vectors, each having dim_lstm components
    lstm_vectors = encoder.predict(X)  

    # Save results in an ARFF file
...
def main():
    ...
    timings["computing_drfm_raw_time_series"] = time.time() - t0

    # Save dynamic RFM-agregate features in ARFF file
    t0 = time.time()
    print("    2- Computing the DRFM-aggregation features")
    save_agregate_features(matricules, nb_transactions, R_time_series, F_time_series, M_time_series, file_agregation)
    timings["computing_drfm_aggregation_features"] = time.time() - t0

    # Save dynamic RFM-LSTM and RFM-agregate-LSTM features in ARFF file
    t0 = time.time()
    print(f"    3- Computing the DRFM-lstm-{dim_lstm} features")
    lstm_vectors = save_lstm_features(matricules, nb_transactions, R_time_series, F_time_series, M_time_series, dim_lstm, file_lstm, nb_epochs, batch_size, patience)
    timings["computing_drfm_lstm_features"] = time.time() - t0
    ...
    #timings["kmeans_dtw"] = time.time() - t0
